...
from app.services.ai_explain import explain_errors
...
def validate_single(
    ...
    explain: bool = Query(False, description="Use Claude to explain validation errors in plain English"),
) -> SingleValidateResponse:
    valid, errors = validator_service.validate_payload(body.schema_name, body.payload, user)
    suggestion = None
    if explain and not valid:
        schema = validator_service.load_schema(body.schema_name, user)
        suggestion = explain_errors(errors, schema)
    return SingleValidateResponse(valid=valid, errors=errors, suggestion=suggestion)
