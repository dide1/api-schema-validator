...
class SQLiteMetadataStore(MetadataStore):
    ...
    def _dump(self) -> None:
        sql_path = self.db_path.replace(".db", ".sql")
...
class PostgreSQLMetadataStore(MetadataStore):
    ...
    @contextmanager
    def _connect(self):
        conn = self._psycopg2.connect(self._url, sslmode="require")
    ...
    def init(self) -> None:
        logger.info("Initialising PostgreSQL metadata store")
        with self._connect() as conn:
    ...
    def _row_to_user(self, row: tuple, keys: list[str]) -> User:
        r = dict(zip(keys, row))
        return User(
            id=r["id"], email=r["email"], name=r["name"],
            role=Role(r["role"]), team_id=r.get("team_id"),
            oauth_provider=r.get("oauth_provider"), oauth_sub=r.get("oauth_sub"),
            created_at=_iso_to_dt(r["created_at"]),
        )

    def _row_to_template(self, row: tuple, keys: list[str]) -> Template:
        r = dict(zip(keys, row))
        return Template(
            template_id=r["template_id"], schema_name=r["schema_name"],
            owner_id=r["owner_id"], visibility=Visibility(r["visibility"]),
            team_id=r.get("team_id"), s3_key=r.get("s3_key"),
            created_at=_iso_to_dt(r["created_at"]), updated_at=_iso_to_dt(r["updated_at"]),
        )

    def _row_to_invite(self, row: tuple, keys: list[str]) -> Invite:
        r = dict(zip(keys, row))
        return Invite(
            token=r["token"], team_id=r["team_id"], invited_by=r["invited_by"],
            created_at=_iso_to_dt(r["created_at"]), expires_at=_iso_to_dt(r["expires_at"]),
        )

    def _row_to_payload(self, row: tuple, keys: list[str]) -> SavedPayload:
        r = dict(zip(keys, row))
        return SavedPayload(
            payload_id=r["payload_id"], payload_name=r["payload_name"],
            owner_id=r["owner_id"], visibility=Visibility(r["visibility"]),
            team_id=r.get("team_id"), content=r["content"],
    ...
    def get_user_by_id(self, user_id: str) -> User | None:
        with self._connect() as conn:
            with conn.cursor() as cur:
                ...
                row = cur.fetchone()
                return self._row_to_user(row, self._cols(cur)) if row else None
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    (user.id, user.email, user.name, user.role.value, user.team_id,
                     user.oauth_provider, user.oauth_sub, _dt_to_iso(created_at)),
                )
        user.created_at = created_at
        return user
    ...
    def update_user(self, user: User) -> User:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "UPDATE users SET email = %s, name = %s, role = %s, team_id = %s WHERE id = %s",
                    (user.email, user.name, user.role.value, user.team_id, user.id),
                )
        return user
    ...
    def list_users(self) -> list[User]:
        with self._connect() as conn:
            with conn.cursor() as cur:
                ...
                cols = self._cols(cur)
                return [self._row_to_user(r, cols) for r in cur.fetchall()]
    ...
    def get_template(self, schema_name: str) -> Template | None:
        safe = schema_name.removesuffix(".json")
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT * FROM templates WHERE schema_name = %s", (safe,))
                row = cur.fetchone()
                return self._row_to_template(row, self._cols(cur)) if row else None

    def list_templates(self) -> list[Template]:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT * FROM templates ORDER BY schema_name")
                cols = self._cols(cur)
                return [self._row_to_template(r, cols) for r in cur.fetchall()]
            ...
            with conn.cursor() as cur:
                cur.execute(
                    """INSERT INTO templates
                       (template_id, schema_name, owner_id, visibility, team_id, s3_key, created_at, updated_at)
                       VALUES (%s, %s, %s, %s, %s, %s, %s, %s)""",
                    (template.template_id, template.schema_name, template.owner_id,
                     template.visibility.value, template.team_id, template.s3_key,
                     _dt_to_iso(template.created_at), _dt_to_iso(template.updated_at)),
                )
        return template
    ...
    def update_template(self, template: Template) -> Template:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """UPDATE templates
                       SET owner_id = %s, visibility = %s, team_id = %s, s3_key = %s, updated_at = %s
                       WHERE schema_name = %s""",
                    (template.owner_id, template.visibility.value, template.team_id,
                     template.s3_key, _dt_to_iso(template.updated_at), template.schema_name),
                )
        return template
    ...
    def delete_template(self, schema_name: str) -> None:
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM templates WHERE schema_name = %s", (safe,))
    ...
    def get_invite(self, token: str) -> Invite | None:
        with self._connect() as conn:
            with conn.cursor() as cur:
                ...
                row = cur.fetchone()
                return self._row_to_invite(row, self._cols(cur)) if row else None

    def list_invites_for_team(self, team_id: str) -> list[Invite]:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT * FROM invites WHERE team_id = %s ORDER BY created_at DESC", (team_id,)
                )
                cols = self._cols(cur)
                return [self._row_to_invite(r, cols) for r in cur.fetchall()]

    def create_invite(self, invite: Invite) -> Invite:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """INSERT INTO invites (token, team_id, invited_by, created_at, expires_at)
                       VALUES (%s, %s, %s, %s, %s)""",
                    (invite.token, invite.team_id, invite.invited_by,
                     _dt_to_iso(invite.created_at), _dt_to_iso(invite.expires_at)),
                )
        return invite
    ...
    def update_invite(self, invite: Invite) -> Invite:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "UPDATE invites SET team_id = %s, invited_by = %s, expires_at = %s WHERE token = %s",
                    (invite.team_id, invite.invited_by, _dt_to_iso(invite.expires_at), invite.token),
                )
        return invite
    ...
    def get_payload(self, payload_name: str) -> SavedPayload | None:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT * FROM payloads WHERE payload_name = %s", (payload_name,))
                row = cur.fetchone()
                return self._row_to_payload(row, self._cols(cur)) if row else None

    def list_payloads(self) -> list[SavedPayload]:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT * FROM payloads ORDER BY payload_name")
                cols = self._cols(cur)
                return [self._row_to_payload(r, cols) for r in cur.fetchall()]
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    (payload.payload_id, payload.payload_name, payload.owner_id,
                     payload.visibility.value, payload.team_id, payload.content,
                     payload.schema_name, _dt_to_iso(payload.created_at), _dt_to_iso(payload.updated_at)),
                )
        return payload
    ...
    def update_payload(self, payload: SavedPayload) -> SavedPayload:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """UPDATE payloads
                       SET visibility = %s, team_id = %s, content = %s, schema_name = %s, updated_at = %s
                       WHERE payload_name = %s""",
                    (payload.visibility.value, payload.team_id, payload.content,
                     payload.schema_name, _dt_to_iso(payload.updated_at), payload.payload_name),
                )
        return payload
    ...
    def delete_payload(self, payload_name: str) -> None:
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM payloads WHERE payload_name = %s", (payload_name,))
